...
//  FrozenCookies.gc_time = Number(localStorage.getItem('frenzyTime'));
  ...
  FrozenCookies.last_gc_state = (Game.frenzy ? Game.frenzyPower : 1) * (Game.clickFrenzy ? Game.clickFrenzyPower : 1);
...
// Press 'm' to pop up a window to mass-buy buildings
...
function upgradeToggle(upgrade, achievements, reverseFunctions) {
  if (!achievements) {
    ...
    achievements.forEach(function(won, index){
      ...
      achievement.won = won;
      if (won && achievement.pool == 'normal') {
...
function autoCookie() {
  if (!FrozenCookies.processing && !Game.OnAscend && !Game.AscendTimer) {
    ...
    if (FrozenCookies.lastHCAmount < currentHCAmount) {
      ...
      FrozenCookies.lastHCTime = Date.now();
      var currHCPercent = (60 * 60 * (FrozenCookies.lastHCAmount - Game.HowMuchPrestige(Game.cookiesReset))/((FrozenCookies.lastHCTime - Game.startDate)/1000));
      if ((Game.HowMuchPrestige(Game.cookiesReset) < (currentHCAmount - changeAmount)) && currHCPercent > FrozenCookies.maxHCPercent) {
